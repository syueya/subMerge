...
export class SourceProxiesComponent extends CmParentComponent implements OnInit {
  ...
  displayedColumns = ['enabled', 'region', 'name', 'type', 'server'];
  ...
  override ngOnInit(): void {
    super.ngOnInit();
    this.search$.pipe(debounceTime(200), distinctUntilChanged(), takeUntil(this.$destroy)).subscribe(raw => {
  ...
  loadProxies(): void {
    this.svc
      .listProxies(this.source.id, true)
      .pipe(takeUntil(this.$destroy), this.trackLoading())
      .subscribe({
        next: res => {
