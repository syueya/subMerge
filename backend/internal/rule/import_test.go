...
ok, errs := parseBatchImportText(text, "DOMAIN", "直连", "个人", "其它")
		...
		if ok[0].Category != "其它" {
			t.Fatalf("row0 category default: %+v", ok[0])
		}
	...
	func TestParseBatchImportText_CategoryColumn(t *testing.T) {
		text := "DOMAIN-SUFFIX,openai.com,美国US,AI-OpenAI,海外AI\nDOMAIN,a.com,直连,备注,PT站\n"
		...
		if ok[0].Category != "海外AI" || ok[0].Note != "AI-OpenAI" {
			t.Fatalf("row0: %+v", ok[0])
		}
		if ok[1].Category != "PT站" || ok[1].Note != "备注" {
