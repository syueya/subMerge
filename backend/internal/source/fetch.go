...
func (s *Service) fetch(rawURL string) (fetchResult, error) {
	...
	if err != nil {
		return fetchResult{}, fmt.Errorf("invalid url: %s", masked)
	}
	...
	if err != nil {
		return fetchResult{}, fmt.Errorf("build request for %s failed", masked)
	}
	...
	if err != nil {
		...
		return fetchResult{}, fmt.Errorf("fetch %s failed: %s", masked, cleanFetchErr(err, rawURL))
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return fetchResult{}, fmt.Errorf("upstream status %d", resp.StatusCode)
	}
	...
	if int64(len(body)) > s.maxBytes {
		return fetchResult{}, fmt.Errorf("response exceeds size limit")
	}
	applog.Info("[fetch] ua=%q status=%d bytes=%d userinfo=%v",
...
func validateFetchURL(u *url.URL) error {
	if u == nil || (u.Scheme != "http" && u.Scheme != "https") || u.Hostname() == "" {
		return fmt.Errorf("unsupported url scheme or host")
	}
