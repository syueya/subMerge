...
func NewHandler(manager ProxyManager, auditSvc *audit.Service) *Handler {
...
func (h *Handler) Save(c *gin.Context) {
	...
	h.audit.Log(middleware.GetUsername(c), "update_outbound_proxy", "outbound_proxy", "save proxy setting", c.ClientIP())
	apiresp.OK(c, view)
}
...
func (h *Handler) Reset(c *gin.Context) {
	...
	h.audit.Log(middleware.GetUsername(c), "reset_outbound_proxy", "outbound_proxy", "reset proxy setting", c.ClientIP())
	apiresp.OK(c, view)
}
